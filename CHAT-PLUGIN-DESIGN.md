...
**v2.6 — doc-grounding + prompt caching on the relay:**
- The `ai` block gains **`system`** (persona/instructions) and **`grounding`** (URL or project-relative file whose text is appended to the system prompt inside `<reference_documentation>`). Grounding resolves **once at startup** so the combined prompt is byte-identical across requests. Demo grounds on `https://manifestx.dev/llms.txt` — answers about Manifest are now accurate instead of invented.
- The relay sends the system prompt as a **`cache_control: ephemeral` block**: identical prefixes hit Anthropic's server-side prompt cache at ~0.1× input price. Verified live (11,968 tokens written on request 1, read from cache on request 2). Note: models have a minimum cacheable prefix (Haiku 4.5: 4096 tokens) below which caching is silently inert — the demo's llms.txt (~2.5K tokens) is under it, which is fine at that size; larger grounding (e.g. `llms-full.txt`, ~130K tokens) is where caching pays.
- Standalone proxy gains cache parity. **Managed Agents noted as a validated future adapter**: Anthropic's stored/versioned agent objects + stateful sessions map 1:1 onto the adapter contract (session=conversation, SSE events=subscribe, reconnect-consolidation=onGap) — the natural backend for a hosted-agents tier; not needed for the docs-bot case.
