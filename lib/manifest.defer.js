...
function register(el, rule, opts) {
    ...
    const rec = el.__mnfstDefer = {
        ...
        urgent: ready, // mounted after ready → the user just made it appear
        rendered: false,
        cleanup: []
    };
    wire(rec);
...
function next() {
    ...
    for (const rec of pending) {
        ...
        if (!best
            || (rec.urgent && !best.urgent)
            || (rec.urgent === best.urgent && (rec.priority < best.priority
...
function schedule() {
    if (idleHandle !== null || !ready || pending.size === 0) return;
    const head = next();
    idleHandle = idle(() => {
        idleHandle = null;
        const rec = next();
