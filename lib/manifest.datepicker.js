		...
		function createState(rootEl) {
			...
			const state = {
				...
				open() {
					...
					this.render();
					// `source` names the invoker. The calendar is body-appended, so without it
					// the popover tree has no idea it belongs to the field's dialog or menu, and
					// opening it light-dismisses the very thing it was opened from. A field can't
					// carry popovertarget (text inputs aren't invokers), so it is declared here.
