                ...
                getMarkdownContent(async (pathOrContent) => {
                    ...
                    if (isFilePath) {
                        try {
                            ...
                            // Prerender classification signal: the resolved
                            // per-locale source, stamped before the fetch so the
                            // renderer sees it even if capture races the request.
                            try { el.setAttribute('data-mnfst-md-src', resolvedPath); } catch (_) { }
                            // Check cache first
