        ...
        const state = {
            ...
            renderLibrary() {
                ...
                if (this.libraryRootValue && window.Alpine?.effect && window.Alpine?.evaluateLater) {
                    ...
                    Alpine.effect(() => {
                        try { Alpine.evaluate(evalCtx, 'typeof $locale !== "undefined" && $locale.current'); } catch {}
        ...
        Alpine.directive('colorpicker', (el, { modifiers, expression }, { cleanup, evaluateLater }) => {
            ...
            if (role === 'swatch') {
                ...
                const wireSwatchTo = (target) => {
                    ...
                    // A generated panel is appended to <body>, so manifest.dialog.css can't
                    // tell from DOM position that it belongs to a swatch inside a dialog
                    // rather than to the page behind it, and fades it out. Say so here.
                    if (el.closest('[popover]')) target.setAttribute('data-popover-nested', '');
                        ...
                        if (isDialog) {
                            ...
                            try {
                                if (target.hasAttribute('popover')) {
                                    // `source` names the swatch as the invoker, so opening the
                                    // panel can't light-dismiss a dialog or menu it sits inside.
                                    if (!target.matches(':popover-open')) target.showPopover({ source: el });
                                } else {
