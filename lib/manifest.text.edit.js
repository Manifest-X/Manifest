...
(function () {
    ...
    // start container instead — as this did — gets the wrong answer whenever the
    // selection starts on an element rather than inside the text: a triple-clicked
    // paragraph reports "not bold", so the toggle wraps what was already wrapped and
    // the nesting cleanup quietly undoes it. Which reads as the button doing nothing.
    function selectionMarked(tag) {
        const r = range(); if (!r) return false;
        if (r.collapsed) return !!ancestor(tag);
        const root = r.commonAncestorContainer;
        const walker = document.createTreeWalker(root.nodeType === 1 ? root : root.parentNode, NodeFilter.SHOW_TEXT);
        let any = false;
        for (let n = walker.nextNode(); n; n = walker.nextNode()) {
    ...
    const toggleInline = (tag, style) => selectionMarked(tag) ? unwrapInline(tag) : wrapInline(tag, style);
    ...
    const inTable = () => !!cellAt();          // caret in a cell, or a block picked by pointer
    ...
    /* ---- Tables ----
       ...
       markup is ordered. */
    let cellDrag = null;                       // a rectangle of cells picked with the pointer
    // A block of cells selected by pointer has no caret in it, so the table commands
    // ask for either — otherwise selecting empty cells and pressing Merge does
    // nothing, which is exactly how it looked.
    const cellAt = () => ancestor('td, th') || (cellDrag && cellDrag.anchor.isConnected ? cellDrag.anchor : null);
    ...
    function mergeCells() {
        ...
        normalizeTables(table.parentElement || table);
        cellDrag = null;                       // the block has become one cell
        paintCells(first.closest('[data-text-edit]') || table);
    ...
    /* An empty cell holds no text, so a text selection can neither reach one nor
       show one. Worse, dragging INTO a trailing empty cell leaves the selection's
       focus clamped to the last position that holds text — which is why the final
       empty cell was always the one left unhighlighted. The pointer is tracked
       instead, and its cell is the far corner. */

    const cellUnder = (x, y) => {
        const el = document.elementFromPoint(x, y);
        const cell = el && el.closest && el.closest('td, th');
        return cell && cell.closest('[data-text-edit]') ? cell : null;
    };
    ...
    function selectedCells() {
        if (cellDrag && cellDrag.anchor !== cellDrag.focus && cellDrag.anchor.isConnected && cellDrag.focus.isConnected) {
            return rectBetween(cellDrag.anchor, cellDrag.focus);
        }
        const table = tableAt(); if (!table) return [];
    ...
    Object.entries(INLINE_TAGS).forEach(([tag, meta]) => {
        COMMANDS[tag] = {
            ...
            active: (v, area) => selectionMarked(tag) || pendingHas(area, tag)
        };
    });
    ...
    // relatedTarget is null for a click onto plain page text; the pointer that caused
    // the blur stands in, and only for the tick that dispatched it.
    let pointerTarget = null;

    // Focus is still the area's own, or on a control that acts on it.
    function keeps(area, node) {
        const el = elementAt(node);
        if (!el) return false;
        if (area.contains(el)) return true;
        const control = el.closest('[data-text-edit-control]');
        return !!control && resolve(control) === area;
    }
    ...
    // A link at the caret comes off; a selection gets one, through the area's own url
    // control where there is one.
    function toggleLink(area) {
        if (ancestor('a')) { run(area, 'a'); return; }
        if (!hasSelection()) return;
        saveRange(area);
        const field = [...controls].find(c => c._te && c._te.id === 'a' && !c._te.page
            && 'value' in c && c.tagName !== 'BUTTON' && resolve(c) === area);
        if (field) { field.focus(); if (field.select) field.select(); return; }
        const url = typeof window.prompt === 'function' ? window.prompt('Link URL', '') : null;
        if (url) run(area, 'a', url);
    }
    ...
    function init() {
        ...
        document.addEventListener('selectionchange', () => {
            ...
            if (a) {
                if (lastFocused && lastFocused !== a) release(lastFocused);   // the one being left owes a null
                saveRange(a); lastFocused = a; report(a);
            }
            else if (lastFocused && editableElsewhere(r.startContainer)) release();
            sync();
        });

        // The caret has moved into something else editable — another plugin's field,
        // a plain contenteditable, an input. Nothing here owns it, so the controls
        // have no target: they must go quiet rather than keep pointing at whichever
        // area was last visited, which is how a command ends up styling text the
        // writer can see is not selected.
    ...
    function report(area) {
        // A caret placed inside one cell ends any block selection.
        const live = sel();
        if (cellDrag && live && live.isCollapsed) { const c = cellOf(live.anchorNode); if (c) cellDrag = null; }
        if (area.querySelector('table')) paintCells(area);
        const shape = selectionOf(area);
        area._selection = shape;
        const px = (n) => Math.round(n) + 'px';
        // Mirrored onto the root as well as the area. A menu anchored to the
        // selection is rarely a descendant of the editor — and once it is a popover
        // it is in the top layer, where nothing inherits from the page at all — so
        // custom properties on the area alone are unreachable from where they are
        // needed. On the root they are readable from anywhere.
        const targets = [area, document.documentElement];
        if (shape && !shape.collapsed) targets.forEach(t => {
            t.style.setProperty('--text-edit-selection-x', px(shape.x));
            t.style.setProperty('--text-edit-selection-y', px(shape.y));
            t.style.setProperty('--text-edit-selection-width', px(shape.width));
            t.style.setProperty('--text-edit-selection-height', px(shape.height));
            t.style.setProperty('--text-edit-selection-center', px(shape.x + shape.width / 2));
        });
        area.toggleAttribute('data-text-edit-selected', !!shape && !shape.collapsed);
        document.documentElement.toggleAttribute('data-text-edit-selected', !!shape && !shape.collapsed);
        area.dispatchEvent(new CustomEvent('text-edit:selection', { bubbles: true, detail: shape }));
    }
    ...
    function area(el, modifiers, expression, { effect, evaluateLater, cleanup }) {
        ...
        el.addEventListener('pointerdown', (ev) => {
            ...
            if (el.querySelector('table')) {
                const hit = borderAt(ev.clientX, ev.clientY);
                if (hit) return startResize(ev, hit);
                const from = cellUnder(ev.clientX, ev.clientY);
                if (from) cellDrag = { anchor: from, focus: from };
            }
        });

        el.addEventListener('pointermove', (ev) => {
            if (!cellDrag || !ev.buttons) return;
            const over = cellUnder(ev.clientX, ev.clientY);
            if (!over || over === cellDrag.focus) return;
            cellDrag.focus = over;
            paintCells(el);
        });

        el.addEventListener('pointerup', () => {
            if (cellDrag && cellDrag.anchor === cellDrag.focus) cellDrag = null;
            paintCells(el);
            sync();
        });
