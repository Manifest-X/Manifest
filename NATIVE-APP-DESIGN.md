...
- ✅ **Full capability set** — `$push` (3 contracts: `request()` author-timed permission, `onToken(fn)`→backend, tap payload→router via `$links`; `onTap()` override), `$app` (lifecycle: reactive `.active`+`onChange`, appStateChange / visibilitychange), `$haptics` (impact/notification/selection/vibrate → `navigator.vibrate` fallback), `$biometric` (available/verify; native-only, web=unsupported), `$camera` (photo/pick → file-input data-URL fallback). All web-verified where testable; native paths (APNs token, tap, Keychain, Face ID, real capture) flagged for on-device verification.
- ✅ **Audit** — 3 parallel adversarial reviews (native JS / loader+build / CSS+`$device`+docs); fixed 2 HIGH (network-init timing dropping the initial `$device.online`; `$links` custom-scheme host drop), 4 MED (store-clobber guards; camera cancel leak; explicit-path Capacitor auto-load; `native`→`utilities` dep), several LOW. Re-verified on `/native`.
- ✅ **Schema + docs + wrap prep** — `"native"` block in `manifest.schema.json`; new **Native Capabilities** docs page (`$device` + all 9 magics, web fallbacks, plugin mapping); `templates/capacitor/` scaffold (config + step-by-step iOS wrap guide).
- ⏭ Remaining: **#8 Capacitor wrap proof** (prep done; needs a Mac/simulator to run) → **#10 payments adapter** (blocked on Nurvana processor).
